...
注：本项目使用AGPL协议，因此如有修改还请开源哦~，建议直接拉取一个request。
# - 使用说明
...
例如第63行为5*60，（距离下课5分钟显示窗口并倒计时），67行为40*60（上课时隐藏窗口，因前一个if先触发，因此倒计时5分钟会直接显示。如要改成对应时间，例：35分钟。及改成35*60），72行的minutes也改成对应时间如35。
